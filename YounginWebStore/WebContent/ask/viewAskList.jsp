...
<H1 align="center"><font face="맑은 고딕" color="#1F497D">물품신청목록</font></H1>
<p align="right">
<c:url var="askURL" value="/Ask"/>
<form method="post" action='${askURL}'>
  <input type="hidden"  name="method" value="searchAskList">
  <select size=1 name="column" style="border-width:1; border-color:rgb(241,243,246); border-style:none;">
    <option selected value="id">순서번호</option>
    <option value="name">학과</option>
    <option value="dat">날짜</option>
    <option value="reqNo">요청수량</option>
  </select>
  <input type="text" name="keyword" style="background-color:white; border-width:1; border-style:solid;">
  <input type=submit value="검색" style="background-color:rgb(241,243,246); border-width:1; border-style:solid;">
 </form>


 </p>
<table border=1 width="700" cellspacing=0 bgcolor="#F1F3F6" bordercolor="white" bordercolordark="white" bordercolorlight="#CCCCCC" align="center">
    <tr>
        <th width="15"><font face="맑은 고딕">NO</font></th>
        <th width="70"><font face="맑은 고딕">날짜</font></th>
        <th width="90"><font face="맑은 고딕">학과</font></th>
        <th width="90"><font face="맑은 고딕">품목</font></th>
        <th width="90"><font face="맑은 고딕">요청수량</font></th>
        <th width="90"><font face="맑은 고딕">처리유무</font></th>
        <th width="90"><font face="맑은 고딕">신청ID</font></th>
	
    </tr>
    
    <c:forEach var="ASK" items="${ASK_LIST}" varStatus="i">
    
    <%-- viewURL 이라는 url 에 /servlet/ask 링크 저장--%>
    <c:url var="viewURL" value="/Ask">
    <%-- 링크에 파라메터 정보를 추가--%>
    <%-- 메서드 파라메터에 viewAsk 대입--%>
    <c:param name="method" value="viewAsk" />
    <c:param name="id" value="${ASK.id}" />
    ...
    <tr>
        ...
        <td>${ASK.dat}</td>
        <td>${ASK.name}</td>
        <td><a href='${viewURL}'>${ASK.dept.name}</a></td>
        <td>${ASK.regNo}개</td>
        <td>${ASK.status.codeName}</td>
        <td>${ASK.writer}</td>
    </tr>
    </c:forEach>
    <%-- 리스트가 없다면 No Data 출력 --%>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${TOTAL_PAGE}
<br><br>
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='${askURL}?method=addAskForm'><font face="맑은 고딕">등록하기</font></a><font face="맑은 고딕">
</font>
